...
local macroCuchilloActivo = false
...
UIElements.TogMacroCuchillo = Tabs.Aim:Toggle({
    Title = "Macro Cuchillo (L2)",
    Desc = "Activa el cuchillo con L2.",
    Callback = function(s) macroCuchilloActivo = s end
})
...
function obtenerCuchillo()
    local char = player.Character
    local backpack = player:FindFirstChild("Backpack")
    local function esCuchillo(item)
        ...
        local n = string.lower(item.Name)
        for i = 1, #forbiddenKeywords do
            if string.find(n, forbiddenKeywords[i]) then return true end
        end
        return false
    end
    if char then
        for _, item in ipairs(char:GetChildren()) do
            if esCuchillo(item) then return item end
        end
    end
    if backpack then
...
function ejecutarAccionMacroCuchillo()
    ...
    local cuchillo = obtenerCuchillo()
    if not cuchillo then return end

    task.spawn(function()
        hum:UnequipTools()
        task.wait()
        hum:EquipTool(cuchillo)
        task.wait(macroEquipDelay)
        if cuchillo.Parent == char then
            -- L2 debe lanzar el cuchillo, no usar Tool:Activate() (que puede acuchillar).
            local throwObj = cuchillo:FindFirstChild("Throw", true)
            if throwObj then
                ...
                elseif throwObj:IsA("BindableFunction") then
                    ...
                    -- Algunos juegos exponen Throw como un objeto interno y
                    -- el LocalScript del cuchillo se encarga del lanzamiento.
                    -- No llamamos Activate() para evitar el ataque cuerpo a cuerpo.
                end
            end
...
runtime.Track(UserInputService.InputBegan:Connect(function(input, processed)
    ...
    -- Macro cuchillo: L2 (ButtonL2), separada de la macro de pistola/R2.
    if macroCuchilloActivo
        and input.UserInputType == Enum.UserInputType.Gamepad1
        and input.KeyCode == Enum.KeyCode.ButtonL2 then
        ejecutarAccionMacroCuchillo()
        return
    end
