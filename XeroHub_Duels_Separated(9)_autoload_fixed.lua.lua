...
local function saveAutoloadSettings()
    ...
    local wrote = pcall(writefile, AUTOLOAD_SETTINGS_FILE, encoded)
    return wrote
end
...
-- Autoload: permite elegir una configuracion guardada y cargarla al iniciar.
Tabs.Config:Toggle({
...
-- Cargar la lista y ejecutar Autoload al iniciar el script.
task.spawn(function()
    task.wait(1)
    refreshConfigs()
