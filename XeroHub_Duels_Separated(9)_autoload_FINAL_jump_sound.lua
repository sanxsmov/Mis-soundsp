...
local function playJumpSound(character)
    if not jumpSoundEnabled then return end
    if jumpSoundId == "" then return end
    if not character then return end

    local parent = character:FindFirstChild("HumanoidRootPart") or character
...
Tabs.Config:Input({
    ...
    Callback = function(Text)
        ...
        elseif value:match("^rbxassetid://") then
            ...
            jumpSoundId = "rbxassetid://" .. value
        end
    end
})

if player and player.Character then
    task.spawn(setupJumpSound, player.Character)
end
