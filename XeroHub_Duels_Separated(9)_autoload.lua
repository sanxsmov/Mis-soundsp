...
do
    ...
    local function renderSnapshot(viewport, mode)
        ...
        if mode == "full" then
            focus += Vector3.new(0, size.Y * 0.04, 0)
        elseif mode == "bust" then
            focus += Vector3.new(0, size.Y * 0.03, 0)
        end
    ...
    local function candidateImageScore(gui)
        ...
        local score = area
        if imageReferencesLocalUser(gui) then score += 100000 end
        return score
    end
...
local function XeroApplyDecodedConfig(decoded, configName)
    if type(decoded) ~= "table" then
        showBottomMessage("Error: configuración inválida.")
        return false
    end
...
if decoded.Animaciones then
    ...
    showBottomMessage("'" .. tostring(configName or "Configuración") .. "' cargada con éxito.")
    return true
end

local function XeroApplyConfigFile(path, configName)
    ...
    return XeroApplyDecodedConfig(decoded, configName)
end
