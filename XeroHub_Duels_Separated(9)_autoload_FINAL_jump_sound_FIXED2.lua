...
-- ==========================================
local WindUI
-- UI separada: se intenta primero el archivo local y después HTTP.
-- Esto evita depender de que game:HttpGet sea el único método disponible.
local XeroEnv = ((getgenv and getgenv()) or _G)
local NOX_UI_URL = XeroEnv.NOX_UI_URL or "https://raw.githubusercontent.com/OnyxDevv/Onyx-web/refs/heads/main/main%20%283%29.lua"

local function XeroFetchUI(url)
    if type(isfile) == "function" and type(readfile) == "function" then
        local localOK, localData = pcall(function()
            if isfile("XeroHub_UI.lua") then
                return readfile("XeroHub_UI.lua")
            end
        end)
        if localOK and type(localData) == "string" and #localData > 100 then
            return localData
        end
    end
    ...
    if type(req) == "function" then
        local reqOK, response = pcall(function()
            return req({
                Url = url,
                Method = "GET",
                Headers = {["User-Agent"] = "XeroHub/3.0"},
            })
        end)
        if reqOK and response then
            local status = tonumber(response.StatusCode or response.Status or 0)
            local body = response.Body
            if (status == 0 or (status >= 200 and status < 300))
                and type(body) == "string" and #body > 100 then
                return body
            end
        end
    end
    ...
    if httpOK and type(data) == "string" and #data > 100 then
        return data
    end

    error("No se pudo descargar XeroHub_UI.lua")
end

local ok, result = pcall(function()
    local source = XeroFetchUI(NOX_UI_URL)

    -- XeroHub UI patch: añade la categoría Sonidos y corrige su numeración.
    source = source:gsub(
    ...
    if type(loadstring) ~= "function" then
        error("Este ejecutor no expone loadstring")
    end
    ...
    local uiResult = chunk()
    if type(uiResult) ~= "table" then
        error("La UI no devolvió la tabla esperada")
    end
    return uiResult
end)
...
if ok and result then
    ...
    warn("[XeroHub] No se pudo iniciar la UI: " .. errorText)
    if startupSplashState.Status then
        startupSplashState.Status.Text = "Error UI: " .. errorText:sub(1, 90)
    end
...
-- ==========================================
-- SONIDO AL SALTAR (SEGURO)
-- ==========================================
local XeroJumpSoundEnabled = false
local XeroJumpSoundId = ""
local XeroJumpConnections = {}

local function XeroPlayJumpSound(character)
    if not XeroJumpSoundEnabled or XeroJumpSoundId == "" then
        return
    end
...
local function XeroSetupJumpSound(character)
    ...
    if XeroJumpConnections[humanoid] then
        XeroJumpConnections[humanoid]:Disconnect()
    end

    XeroJumpConnections[humanoid] = runtime.Track(humanoid.StateChanged:Connect(function(_, newState)
        if newState == Enum.HumanoidStateType.Jumping then
            XeroPlayJumpSound(character)
        end
    end))
end

-- La interfaz usa el mismo sistema de Input que el resto de XeroHub.
pcall(function()
    Tabs.Config:Section({ Title = "Sonidos" })
    ...
    Tabs.Config:Toggle({
        ...
        Value = false,
        Callback = function(Value)
            XeroJumpSoundEnabled = Value == true
        end
    })

    Tabs.Config:Input({
        Title = "ID del Sonido al Saltar",
        Placeholder = "Ej: 123456789",
        Callback = function(Text)
            local value = tostring(Text or ""):gsub("%s+", "")
            if value == "" then
                XeroJumpSoundId = ""
            elseif value:match("^rbxassetid://") then
                XeroJumpSoundId = value
            else
                XeroJumpSoundId = "rbxassetid://" .. value
            end
        end
    })
end)
...
if player and player.Character then
    task.spawn(XeroSetupJumpSound, player.Character)
end
