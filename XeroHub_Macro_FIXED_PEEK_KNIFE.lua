...
-- L2 queda consumido para que el juego no lo use para Shift Lock.
-- En móvil, el mismo Action crea un botón táctil independiente.
...
-- La secuencia es deliberadamente no-bloqueante: puedes seguir caminando,
-- asomarte, lanzar y volver a sacar la pistola sin detener el movimiento.
...
lanzarCuchilloRapido = function()
    ...
    task.spawn(function()
        pcall(function()
            ...
            if not (knife and humanoid and humanoid.Health > 0) then
                return
            end

            -- Guardamos el arma actual para volver a ella después del lanzamiento.
            if previousTool == knife then
                previousTool = nil
            end

            -- Equipado rápido, sin tocar WalkSpeed/MoveDirection.
            humanoid:EquipTool(knife)

            local equipDeadline = os.clock() + math.max(knifeEquipDelay, 0.02)
            repeat
                task.wait()
            until knife.Parent == character or os.clock() >= equipDeadline

            -- El margen sigue siendo pequeño para permitir que KnifeClient se inicialice.
            task.wait(math.max(0, knifeEquipDelay))

            -- Activación normal de la Tool: no requiere congelar al jugador.
            pcall(function()
                knife:Activate()
            end)
            ...
            -- Volver a la pistola anterior si seguía siendo válida.
            -- Esto permite el patrón: asomarse -> L2 -> cuchillo -> pistola -> esconderse.
            if previousTool and previousTool.Parent == character then
                pcall(function()
                    humanoid:EquipTool(previousTool)
                end)
            elseif previousTool and previousTool.Parent == player:FindFirstChildOfClass("Backpack") then
                pcall(function()
                    humanoid:EquipTool(previousTool)
                end)
            end
...
runtime.Track(UserInputService.InputBegan:Connect(function(input)
    if not knifeMacroEnabled then return end
    if input.KeyCode ~= Enum.KeyCode.ButtonL2 then return end
    lanzarCuchilloRapido()
end))

-- El mismo botón táctil creado por ContextActionService usa la secuencia de L2.
-- Solo se muestra cuando la macro está activa.
local knifeTouchVisibilityConnection = runtime.Track(RunService.RenderStepped:Connect(function()
    if knifeTouchButton then
        knifeTouchButton.Visible = knifeMacroEnabled
    end
end))
