...
-- Aim Assist suave para control: solo mientras se apunta con L2.
local aimAssistEnabled = false
local aimAssistConnection = nil
local aimAssistStrength = 0.08
...
local function getAimAssistTarget()
    ...
    for _, other in ipairs(Players:GetPlayers()) do
        if other ~= player and (not isEnemy or isEnemy(other)) then
...
local function setAimAssist(enabled)
    ...
    if aimAssistEnabled then
        aimAssistConnection = RunService.RenderStepped:Connect(function()
            ...
            if not UserInputService:IsGamepadButtonDown(Enum.UserInputType.Gamepad1, Enum.KeyCode.ButtonL2) then return end
            if knifeMacroEnabled then return end
            local camera = workspace.CurrentCamera
            ...
            camera.CFrame = camera.CFrame:Lerp(desired, aimAssistStrength)
        end)
    end
end

UIElements.TogAimAssist = Tabs.Aim:Toggle({
    Title = "Aim Assist (L2)",
    Desc = "Acerca suavemente la mira al enemigo mientras apuntas con L2.",
    Callback = function(v)
        setAimAssist(v)
        markAutoConfigChanged()
    end
...
local lanzarCuchilloRapido
local knifeTouchButton = nil
...
local function setKnifeTouchVisible(visible)
    if knifeTouchButton then
        knifeTouchButton.Visible = visible == true
    end
end
...
local function setKnifeL2Block(enabled)
    if enabled and not knifeL2BlockBound then
        local ok = pcall(function()
            ContextActionService:BindActionAtPriority(
                ...
                function(_, inputState)
                    if inputState == Enum.UserInputState.Begin and lanzarCuchilloRapido then
                        lanzarCuchilloRapido()
                    end
        ...
        if ok then
            task.defer(function()
                pcall(function()
                    knifeTouchButton = ContextActionService:GetButton(knifeL2ActionName)
                    if knifeTouchButton then
                        knifeTouchButton.Name = "XeroHub_KnifeTouch"
                        knifeTouchButton.Text = "🔪"
                        knifeTouchButton.Size = UDim2.fromOffset(62, 62)
                        knifeTouchButton.Position = UDim2.new(1, -145, 1, -205)
                        knifeTouchButton.BackgroundTransparency = 0.18
                        knifeTouchButton.Visible = true
                    end
                end)
            end)
        end
    elseif not enabled and knifeL2BlockBound then
        pcall(function() ContextActionService:UnbindAction(knifeL2ActionName) end)
        knifeL2BlockBound = false
        knifeTouchButton = nil
    end
    setKnifeTouchVisible(enabled)
end

UIElements.TogKnifeMacro = Tabs.Aim:Toggle({
    Title = "Activar Macro Cuchillo (L2)",
    Desc = "L2/táctil lanza el cuchillo; no hace ataque cuerpo a cuerpo.",
    Callback = function(v)
...
-- Intenta el evento de lanzamiento real del Tool. Si el juego no expone un
-- RemoteEvent "Throw", usa la activación normal como respaldo.
local function lanzarToolComoProyectil(knife)
    local throwRemote = knife:FindFirstChild("Throw", true)
    if throwRemote and throwRemote:IsA("RemoteEvent") then
        local camera = workspace.CurrentCamera
        local targetPosition = camera and (camera.CFrame.Position + camera.CFrame.LookVector * 1000) or Vector3.new()
        local ok = pcall(function()
            throwRemote:FireServer(targetPosition)
        end)
        if ok then return true end
        ok = pcall(function() throwRemote:FireServer() end)
        if ok then return true end
    end

    local throwBindable = knife:FindFirstChild("Throw", true)
    if throwBindable and throwBindable:IsA("BindableEvent") then
        pcall(function() throwBindable:Fire() end)
        return true
    end

    return pcall(function() knife:Activate() end)
end

lanzarCuchilloRapido = function()
    ...
    task.spawn(function()
        pcall(function()
            local character = player.Character
            local humanoid = character and character:FindFirstChildOfClass("Humanoid")
            local previousTool = character and character:FindFirstChildOfClass("Tool")
            local knife = obtenerCuchilloMacro()
            if not (knife and humanoid and humanoid.Health > 0) then return end
            if previousTool == knife then previousTool = nil end

            humanoid:EquipTool(knife)
            local deadline = os.clock() + math.max(knifeEquipDelay, 0.02)
            repeat task.wait() until knife.Parent == character or os.clock() >= deadline
            task.wait(math.max(0, knifeEquipDelay))

            lanzarToolComoProyectil(knife)
            task.wait(math.max(0, knifeThrowDelay))
...
runtime.Track(UserInputService.InputBegan:Connect(function(input)
    if input.KeyCode == Enum.KeyCode.ButtonL2 and knifeMacroEnabled then
        lanzarCuchilloRapido()
    end
end))

runtime.Track(RunService.RenderStepped:Connect(function()
    if knifeTouchButton then
        knifeTouchButton.Visible = knifeMacroEnabled and UserInputService.TouchEnabled
    end
end))
